...
### 2. 企业微信参数 (核心难点)
这里分为两组参数，分别负责“发消息”和“收指令”：
- **推送参数 (负责发卡片)**:
  - **CorpID (企业ID) / AgentID (应用ID) / CorpSecret (应用密钥)**: 缺一不可。系统通过这三个参数获取权限，主动向你的企业微信发送 NAS 状态图文卡片。
- **回调参数 (负责接收你的控制指令)**:
  - **Token / EncodingAESKey**: ⚠️ **关键**：这两个参数用于**企业微信 Webhook 的双向验证与解密**。只有配置正确，你在微信里点击“系统概览”或发送“风扇 1”时，NAS 才能正确接收并执行操作。
  - **应用主页 / 专属图片**: 配置收到推送点击后跳转的 NAS 真实地址，以及微信图文卡片的展示封面图。
